...
module {
    ...
    // Creates a List with the default supported standards and returns it.
    public func init_standards() : List.List<MigrationTypes.Current.SupportedStandard> {
        let standards = List.empty<MigrationTypes.Current.SupportedStandard>();
        List.add(standards, default_standard);
        standards;
    };
    ...
    public func accountToValue(acc : MigrationTypes.Current.Account) : MigrationTypes.Current.Value {
        let list = List.empty<MigrationTypes.Current.Value>();
        List.add(list, #Blob(Principal.toBlob(acc.owner)));
        switch(acc.subaccount){
          case(null){};
          case(?val){
            List.add(list, #Blob(val));
          };
        };
    ...
    public func insert_map(top : ?Value, key : Text, val : Value) : Result.Result<Value, Text> {
        ...
        switch (foundTop) {
          case (#Map(a_map)) {
            let listMap = List.empty<(Text, Value)>();
            var bFound = false;
            for(thisItem in a_map.vals()){
              if(key == thisItem.0){
                List.add<(Text, Value)>(listMap, (key, val));
                bFound := true;
              } else {
                List.add<(Text, Value)>(listMap, thisItem);
              };
            ...
            return #ok(#Map(List.toArray(listMap)));
          };

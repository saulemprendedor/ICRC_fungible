...
module {
  ...
  public type ICRC85State = OVSTypes.ICRC85State;
  ...
  /// Represents an account on the ledger, comprising an owner (principal) and an optional subaccount identifier.
    public func ahash(b : Blob) : Nat32 {
    var h : Nat32 = 5381;
    for (byte in Blob.toArray(b).vals()) {
      h := ((h << 5) + h) + (Nat32.fromNat8(byte));
    };
    h
  };

  public func account_hash32(acc : Account) : Nat32 {
    var h = ahash(Principal.toBlob(acc.owner));

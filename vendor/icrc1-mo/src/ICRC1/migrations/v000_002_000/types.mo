...
// v0_2_0: Migrated from mo:map/mo:vector to mo:core/Map/mo:core/List
...
module {
  ...
  /// `ICRC85State`
  ///
  /// Represents the state for ICRC-85 Open Value Sharing (OVS).
  /// Tracks active actions (transactions) and scheduling for cycle sharing.
  public type ICRC85State = OVSTypes.State;

  /// `ICRC85Environment`
  ///
  /// Environment configuration for ICRC-85 Open Value Sharing.
  /// Allows customization of cycle sharing behavior.
  public type ICRC85Environment = OVSTypes.Environment;
  ...
  public type State = {
    ...
    var metadata : ?Value;
    var supported_standards : ?List.List<SupportedStandard>;
    var local_transactions : List.List<Transaction>;
    ...
    var org_icdevs_ovs_fixed_state : ICRC85State;
  ...
  /// Uses mo:core/Map with account_compare for ordering.
  ...
  public type TextMap = [(Text, Value)];

  public type AdvancedEnvironment = {
      /// ICRC-85 Open Value Sharing configuration
      icrc85: {
        kill_switch: ?Bool;
        handler: ?(([(Text, TextMap)]) -> ());
        tree: ?[Text];
        collector: ?Principal;
        advanced: ?OVSTypes.AdvancedEnvironment;
      };
      get_fee : ?((State, Environment, TransferArgs) -> Balance);
      /// Controls fee validation behavior:
      /// - #Strict: Fee must exactly match the calculated fee (ICRC-1 compliant, default)
      /// - #Tolerant: Fee can be greater than or equal to the calculated fee
      fee_validation_mode: ?FeeValidationMode;
    };
  ...
  public type Environment = {
    ...
    var org_icdevs_class_plus_manager : ?ClassPlus.ClassPlusInitializationManager;
  };

  /// `FeeValidationMode`
  ///
  /// Determines how the ledger validates fees in transfer requests.
  /// - #Strict: Per ICRC-1 spec, the fee must exactly match the expected fee (returns BadFee otherwise)
  /// - #Tolerant: Allows fees greater than or equal to the expected fee (for backwards compatibility)
  public type FeeValidationMode = {
    #Strict;
    #Tolerant;
  };
  ...
  /// `TxLog`
  ///
  /// A list holding a log of transactions for the ledger.
  ...
  /// `account_compare`
  ///
  /// Orders two `Account` instances. Used with mo:core/Map.
  ///
  /// Parameters:
  /// - `a`: First `Account` to compare.
  /// - `b`: Second `Account` to compare.
  ...
  /// `blob_compare`
  ///
  /// Orders two Blob instances. Used with mo:core/Map for recent_transactions.
  public func blob_compare(a : Blob, b : Blob) : Order.Order {
    Blob.compare(a, b)
  };
  ...
  ///
  /// Represents a callback function type that notifiers will implement to be alerted to test token transfer events.
